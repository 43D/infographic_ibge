...
! function(t) {
    function o() { var t = (new Date).getTime(); return "undefined" != typeof performance && "function" == typeof performance.now && (t += performance.now()), "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function(o) { var e = (t + 16 * Math.random()) % 16 | 0; return t = Math.floor(t / 16), ("x" === o ? e : 3 & e | 8).toString(16) }) }
    window.JSMaps = window.JSMaps || { maps: {} }, Array.prototype.find || Object.defineProperty(Array.prototype, "find", {
        value: function(t) {
    ...
    Raphael.el.trigger = function(t) { for (var o = 0, e = this.events.length; o < e; o++) this.events[o].name == t && this.events[o].f.call(this) }, t.fn.JSMaps = function(a) {
        function s(o, e, i) { t.each(e, function(t, e) { o[e].animate({ fill: i }, 500) }) }

        function n(o) { for (var e, i = 0, a = o.length; i < a; i++) e = o[i], t.each(D.groups, function(o, i) { t.each(i.members, function(t, o) { e.name === o && (e.group = i.name, e.color = i.color, e.hoverColor = i.hoverColor, e.selectedColor = i.selectedColor) }) }); return o }
        ...
        function l() {
            function o() { P = d.parent().width(), w = x.width(), "text" === D.stateClickAction ? "bottom" === (h = w >= 767 ? D.textPosition : "bottom") ? (Z = (Y = P) / F, d.css({ width: Y + "px", height: Z + M.height() + "px" }), M.css({ width: Y + "px", marginTop: Z + "px", height: "auto" })) : (Y = P - D.textAreaWidth, Z = Y / F, d.css({ width: w >= 767 ? Y + D.textAreaWidth + "px" : Y + "px", height: Z + "px" }), M.css({ width: w >= 767 ? D.textAreaWidth + "px" : Y + "px", height: w >= 767 ? Z + "px" : D.textAreaHeight, display: "inline", float: w >= 767 ? D.textPosition : "none", marginTop: w >= 767 ? 0 : Z + "px" })) : (Z = (Y = P) / F, d.css({ width: Y + "px", height: Z + "px" })), u.setSize(Y, Z) }

            function a(o) { i && e || D.disableTooltip || (p(), T.after(t("<div />").addClass("jsmaps-tooltip")), t(".jsmaps-tooltip").html(o), l(), t(".jsmaps-tooltip").fadeIn()) }

            function l() { j = -40, k = v - t(".jsmaps-tooltip").height() + j < 0, j = k ? 40 : j - t(".jsmaps-tooltip").height(), t(".jsmaps-tooltip").css({ left: f - t(".jsmaps-tooltip").width() / 2, top: v + j }) }

            function p() { T.next(".jsmaps-tooltip").remove() }
            ...
            D.displayMousePosition && (t('<div class="jsmaps-mouse-position"><div class="xPos">X: 0</div><div class="yPos">Y: 0</div></div>').appendTo(d), t("body").css("cursor", "crosshair")), D.displayViewBox && t('<div class="jsmaps-viewbox-data"><div class="xPos">X: {0}</div><div class="yPos">Y: {0}</div><div class="zoom">Zoom: {0}</div></div>').appendTo(d), "text" === D.stateClickAction && ((M = t('').appendTo(d)).html(D.defaultText), "left" === D.textPosition && (T.css({ left: "auto", right: "0" }), H && H.css({ left: "auto", right: "10px" }))), d.mousemove(function(t) { O(t) }),
                function() {
                    function e(o) {
                        var e = this.data("id"),
                            i = !!this.data("group"),
                    ...
                    var i, l, d, m, h = { "stroke-width": D.strokeWidth || 1 },
                        f = { "font-family": "Arial, sans-serif", "font-weight": "bold", "font-size": D.abbreviationFontSize, fill: D.abbreviationColor, "z-index": 1e3 },
                        v = { fill: "#f00", "stroke-width": 0, opacity: 0 };
                    D.groups && D.groups.length && (t.each(D.groups, function(t, o) { o.set = u.set(), o.groupIds = [] }), I = n(I));
                    for (var x = 0, w = I.length; x < w; x++) {
                        I[x].id = x, h = I[x].enable ? t.extend(h, { fill: I[x].color, stroke: D.strokeColor, id: x }) : t.extend(h, { fill: D.offColor, stroke: D.offStrokeColor }), v = t.extend(v, { cursor: I[x].enable ? D.displayMousePosition ? "crosshair" : "pointer" : "default" }), i = u.path(I[x].path).attr(h), A.push(i), (I[x].enable && D.displayAbbreviations || !I[x].enable && D.displayAbbreviationOnDisabledStates) && (D.autoPositionAbbreviations ? (d = (l = i.getBBox()).x + l.width / 2 + I[x].textX, m = l.y + l.height / 2 + I[x].textY) : (d = I[x].textX, m = I[x].textY), C.push(u.text(d, m, I[x].abbreviation).attr(f)));
                        var j, k, P;
                        I[x].group ? ((k = (j = D.groups.find(function(t, o) { if (I[x].group === t.name) return P = o, t.groupIds.push(x), t })).set.push(u.path(I[x].path)).attr(v)).data("group", j), k.data("id", P), k.name = j.name, k.enable = j.enable) : ((k = u.path(I[x].path).attr(v)).data("id", x), k.name = I[x].name, k.enable = I[x].enable), y.push(k), k.mouseover(e), k.mouseout(e), k.click(function(o) {
                            if (!S || !S.isDragging()) {
                    ...
                    D.displayMousePosition || (o(), u.setViewBox(0, 0, D.mapWidth, D.mapHeight), D.responsive && t(window).on("resize", function() { o() }))
                }(),
                function() { C.forEach(function(t) { t.toFront() }), y.forEach(function(t) { t.toFront() }) }(), X && X.length && function() {
                    function o(o) {
                        var e = this.data("id"),
                            i = X[e],
                            n = "mouseover" === o.type,
                    ...
                    for (var e = 0; e < X.length; e++) {
                        X[e].id = e;
                        var i, n = { cursor: "pointer", fill: X[e].color, stroke: D.strokeColor, id: e };
                        if (X[e].src && "" !== X[e].src) {
                            ...
                            i.animate({ transform: "T" + d + "," + m + " S" + l }, 0)
                    ...
                    t("body").on("mousewheel DOMMouseScroll", function(o) { t(o.target).parents(".jsmaps").length && (o.preventDefault(), o.stopPropagation()) }), (S = u.panzoom({ displayViewBox: D.displayViewBox, initialZoom: D.initialZoom, initialPosition: { x: D.initialMapX, y: D.initialMapY }, originSize: { width: D.mapWidth, height: D.mapHeight } })).enable(), H.on("click", function(o) {
                    ...
                    var e = y.sort(function(t, o) { return t.name < o.name ? -1 : t.name > o.name ? 1 : 0 });
                    (e = e.filter(function(t) { return t.enable })).forEach(function(o) { t("<option>").val(o.name).text(o.name).appendTo(E) }), E.on("change", function() { "default" !== this.value ? d.trigger("stateClick", this.value) : d.trigger("stateUnClick") })
                }(), m && m.length && m.fadeOut(), d.fadeIn()
        }

        function p() { u.remove(), d.find(".jsmaps-console").length && d.find(".jsmaps-console").remove(), d.find(".jsmaps-mouse-position").length && d.find(".jsmaps-mouse-position").remove(), d.find(".jsmaps-text").length && d.find(".jsmaps-text").remove(), d.siblings(".jsmaps-select").length && d.siblings(".jsmaps-select").remove(), S && (S = null), A = [], y = [], C = [], l() }
        var c = t.extend({ responsive: !0, offColor: "#ccc", strokeColor: "#24221f", offStrokeColor: "#444", strokeWidth: 1, abbreviationColor: "#f2f2f2", abbreviationFontSize: 12, displayAbbreviations: !0, displayAbbreviationOnDisabledStates: !1, autoPositionAbbreviations: !1, stateClickAction: "text", textPosition: "right", hrefTarget: "_blank", textAreaWidth: 300, textAreaHeight: 300, pinSize: 10, displayMousePosition: !1, displayViewBox: !1, enablePanZoom: !1, mapFolder: "maps/", initialZoom: 0, initialMapX: 0, initialMapY: 0, retainPanZoomOnRedraw: !1, displayPreloader: !0, preloaderText: "Loading map...", disableTooltip: !1, selectElement: !0, selectElementDevices: ["mobile"], selectElementDefaultText: "Please select", onReady: function() {}, onStateClick: function() {}, onStateOver: function() {}, onStateOut: function() {} }, a);
        ...
        var M, S, A = [],
            ...
            I = O.paths,
            X = O.pins;
        l(), d.on("stateClick", function(o, e) { t.each(y, function(t, o) { pathName = o.name, e === pathName && ("set" === y[t].type ? y[t][0] : y[t]).trigger("click") }) }), d.on("stateUnClick", function() {
            if (g) {
                var t = g.groupIds || [g.id];
                s(b ? W : A, t, g.color), "text" === D.stateClickAction && M.html(D.defaultText), g = null
            }
        }), d.on("reDraw", function(o, e) { e && (e.config && (D = t.extend(D, e.config)), S && D.retainPanZoomOnRedraw && (D.initialZoom = S.getCurrentZoom(), D.initialMapX = S.getCurrentPosition().x, D.initialMapY = S.getCurrentPosition().y), e.pins && (X = e.pins), e.paths && (I = e.paths), m && m.length ? m.fadeIn("fast", p) : p()) }), c.onReady.call(this)
    }
}(jQuery);